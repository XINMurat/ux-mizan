...
| U12 | an open finding carries a `review_by`; once it passes, the finding must record a decision — extend, park, or close. W4 warned about this since 0.1; U12 is the date that makes the warning terminal |
| U13 | a defect that exists only while **two** flows are active at once names both of them: the pair, the guarantee that breaks, and the safe order when the pair is asymmetric. The parent must be the heavier of the two — severity reads the weight from the parent, so a free choice of parent would be a free choice of severity |
